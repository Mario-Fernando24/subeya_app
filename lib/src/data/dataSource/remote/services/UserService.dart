...
class Userservice {
  ...
  Userservice(this.token);
    ...
    Future<Resource<AuthResponse>> updateWithImage(int id, User user, File file ) async {

      try {
        final authToken = await token;
        final String url = '${Apiconfig.baseUrl}/users/apload/$id';

        print('================= ACTUALIZAR USUARIO CON IMAGEN =================');
        print('➡️ URL: $url');
        print('🔑 Token: $authToken');
        print('📦 Body: ${user.toJson()}');
